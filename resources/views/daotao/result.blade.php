        ...
        <div class="card card-secondary">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <form method="GET" class="d-flex align-items-center gap-2">
                    ...
                    {{-- Sort điểm --}}
                    <select name="sort" class="form-control form-select" style="width: 220px;">
                    ...
                    <div class="input-group input-group-sm" style="width: 320px;">
                        <input type="text" name="q" class="form-control" placeholder="Tìm tên/email/CCCD"
                            value="{{ $q }}">
                        <button class="btn btn-primary" type="submit"><i class="fas fa-search"></i></button>
                    </div>

                    <a class="btn btn-success btn-sm" href="{{ route('wishes.accepted.exportAll') }}">
                    ...
                    <tbody>
                        @forelse($wishes as $i => $w)
                            <tr>
                                ...
                                <td>{{ $w->major_name ?? '—' }}</td>
                                <td class="text-end">{{ is_null($w->score_d01) ? '—' : number_format($w->score_d01, 2) }}
                                </td>
                                <td class="text-nowrap">{{ \Carbon\Carbon::parse($w->updated_at)->format('d/m/Y H:i') }}
                                </td>
                            </tr>
                        @empty
                            <tr>
